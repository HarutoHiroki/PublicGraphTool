...
d3.json(watermark_animation + "/metadata.json").then(function(animData){
    // Get the animation going
    let frameNo = 1;

    // Change frame every X ms (depends on metadata delay)
    setInterval(function(){
            frameNo = frameNo < animData.frames ? ++frameNo : 1;
            gr.select("[id=background]")
                .attrs({"xlink:href":watermark_animation + "/frame"+frameNo+".svg"}); 
                // "folder/frame[frameNo].svg"
        }, animData.delay
    );
});
...
function themeChooser(command) {
    let docBody = document.querySelector("body"),
        ...
        themeState = localStorage.getItem("theme-pref") || "theme-default";

    // If a change event, make changes to state
    if (command === "change") {
        themeState = themeState === "theme-light" ? "theme-contrast" : themeState === "theme-contrast" ? "theme-default" : "theme-light";
    }

    // Apply state
    localStorage.setItem("theme-pref", themeState);
    docBody.classList.remove("theme-light", "theme-contrast", "theme-default");
    docBody.classList.add(themeState);
    themeButton.setAttribute("current-theme", themeState);
    themeButton.textContent = themeState === "theme-light" ? "contrast mode" : themeState === "theme-contrast" ? "dark mode" : "light mode";
}
